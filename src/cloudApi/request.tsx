...
export default class CloudRequest {
    cloud: any;

    async call(obj, number = 0) {
        const that = this
        if (that.cloud == null) {
            that.cloud = new Taro.cloud.Cloud({
                resourceAppid: 'wxa688e0cca18e3f1f', // 微信云托管环境所属账号，服务商appid、公众号或小程序appid
                resourceEnv: 'prod-5gkhjln1fb14c277', // 微信云托管的环境ID
            })
        ...
        try{
            const result = await that.cloud.callContainer({
                ...
                header: {
                    'X-WX-SERVICE': 'springboot-7k2z-001', // xxx中填入服务名称（微信云托管 - 服务管理 - 服务列表 - 服务名称）
                    // 其他header参数
                }
                // 其余参数同 wx.request
            })
